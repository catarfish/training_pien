...
model{
  for(i in 1:N){
    y[i] ~ dbin(p[i], n[i])
    y.rep[i] ~ dbin(p[i], n[i])
    logit(p[i]) <- beta[1, m[i]] + beta[2, m[i]] * x[i] + eps[s[i]]
  }
  for(p in 1:2){
    for(j in 1:Nm){
        beta[p, j] ~ dnorm(0, 0.00001)
...
datlist <- list(y = moths$removed, 
                ...
                Ns = length(unique(moths$site)))


### Fit data to model using MCMC
# Compile model object
jm <- jags.model(file = textConnection(mstring2),
                 data = datlist,
                 n.chains = 3)
